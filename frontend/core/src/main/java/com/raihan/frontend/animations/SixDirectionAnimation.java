...
public class SixDirectionAnimation implements com.raihan.frontend.animations.Animation {
    private final Texture spriteSheet;
    private final Animation<TextureRegion>[] directionalAnimations;

    public SixDirectionAnimation(String filePath, int frameCols, int frameRows, float frameDuration, boolean LOOP) {
    ...
    // Di dalam kelas DirectionalAnimation.java
    ...
    private int getDirectionIndex(float facingX, float facingY) {
        ...
        else if (facingX > 0) return 5;
        else if (facingX < 0) return 1;
